...
function startTest() {
  testStartTime = new Date(); // 🕒 Capture start time
  testMode = document.body.classList.contains("dark-mode") ? "dracula" : "eclipse";
...
function endTest() {
  ...
  const time_taken = Math.round((endTime - testStartTime) / 1000); // in seconds
  localStorage.setItem("duration", time_taken); //  Save to localStorage
  ...
  // Step 1: Save typing test with start/end time
  axios.post("http://localhost:5000/typing-test/complete", {
    userID: user.UserID,
    test_mode: testMode,
    programming_language: testLanguage,
    ...
    // Step 2: Save result
    return axios.post("http://localhost:5000/save-result", {
      testID,
      accuracy,
    ...
    console.error("Error saving test or result:", error);
...
exitButton.onclick = () => {
  ...
  if (confirmExit) {
    window.location.href = "http://localhost:3000/home"; // Go back to homepage or change URL if needed
  } else {
    // Resume timer if testDuration is not unlimited
    if (testDuration !== "unlimited") {
      timer = setInterval(() => {
        timeLeft--;
        timerDisplay.textContent = timeLeft;
